...
checkpoint quarantine by an oversized-object integrity check. It adopts only
the uncounted suffix, merges its catalog records idempotently, and advances
without re-downloading or re-uploading the messages. Recovery validates commit
membership plus every referenced object's location, name, type, and byte count,
but does not full-read message bodies.

S3/R2 APPLY uses one-message transactions and a 1 MiB parallel payload bound
after larger live combinations exhausted the Apps Script V8 heap. This avoids
repeatedly losing multi-message work in attachment-heavy mailbox regions.
Messages that cannot fit individually still enter the bounded retry and
dead-letter path.

- Existing R2/S3 objects above the bounded replay-hash limit are no longer
  loaded wholesale into Apps Script. They fail fast into durable retry and
  dead-letter isolation, leaving the object untouched for external attestation.

## What was added in 1.3.0-dev.20

- Integrity-verified R2/S3 replay recovery now covers large messages above the
  parallel upload-payload ceiling as well as normal parallel waves.
- The existing object is still adopted only after byte/hash validation; a
  mismatch is quarantined and recreated under the same checkpoint.

## What was added in 1.3.0-dev.19
...
- Healthy S3/R2 transactions and request waves now contain up to twenty
  messages while upload payload memory remains capped at 8 MiB.
- Failed multi-message checkpoints still replay one exact message at a time,
  preserving bounded retries and dead-letter isolation.

## What was added in 1.3.0-dev.16

- S3/R2 APPLY now groups up to eight healthy messages into one durable batch,
  amortizing the deterministic commit, catalog merge, and final checkpoint.
- Upload waves remain bounded to eight requests and 8 MiB, while any failed
  multi-message checkpoint still replays one message at a time for safe retry
  and dead-letter isolation.

## What was added in 1.3.0-dev.15
...
S3/R2 APPLY processes one message per durable transaction and reads or
