...
(function run() {
  ...
  assert.strictEqual(sandbox.__BACKUP_CONFIG.VERSION, '1.3.0-dev.28');
  ...
  assert.strictEqual(sandbox.__BACKUP_CONFIG.S3_MAX_PARALLEL_BYTES, 2 * 1024 * 1024);
  assert.strictEqual(sandbox.__BACKUP_CONFIG.S3_APPLY_BATCH_SIZE, 4);
  ...
  assert.strictEqual(sandbox.GmailBackupLibrary.version(), '1.3.0-dev.28');
  ...
  const boundedConflictRuntime = sandbox.GmailBackupLibrary.createRuntime({
    config: {
      STORAGE_BACKEND: 'S3', ARCHIVE_ENCODING: 'EML',
      S3_REPLAY_FULL_HASH_MAX_BYTES: 1,
    },
    services: {drive: sandbox.DriveApp},
  });
  assert.throws(function () {
    sandbox.GmailBackupLibrary.withRuntime(boundedConflictRuntime, function () {
      sandbox.recoverS3ConditionalCreate_({
        id: 'parallel-conflict', archive: conflictArchive,
        canonical: {byId: {}, allById: {}}, rootFolder: root,
      }, {id: conflictFile.getId()}, {metrics: sandbox.newOperationMetrics_()});
    });
  }, function (error) {
    return error && error.code === 'S3_REPLAY_ATTESTATION_REQUIRED';
  });
  ...
  // Simulate a stale overlapping worker regressing the durable cursor into
  // the middle of the already-published [0, 2) commit. Recovery must adopt
  // only the uncounted suffix and must not full-read every committed object.
  ...
  assert.strictEqual(machineStatus.exporterVersion, '1.3.0-dev.28');
  assert.strictEqual(
